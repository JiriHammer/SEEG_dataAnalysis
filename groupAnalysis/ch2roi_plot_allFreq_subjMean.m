function ch2roi_plot_allFreq_subjMean(GRUP, groupInfo)
...
for freq = 1:size(list_FB,1)
    ...
    for aa = 1:size(list_AA,1)
        ...
        if ~isempty(GRUP.trials_subjMean{aa,freq})
            if size(unique(GRUP.nSubj_roi{aa,freq,1}),1) >= groupInfo.minSubj
                ...
                y_avg = mean(GRUP.trials_subjMean{aa,freq}(i_t,:,:),2);
                y_sem = sem(GRUP.trials_subjMean{aa,freq}(i_t,:,:),2);
    ...
    %                     plotband(xlim, [B_avg,B_avg], [B_sem,B_sem], 'g');
                ...
                if isfield(GRUP, 'H_trials_subjMean')
                    plot2axes_signif_filled(GRUP.time{aa,freq}(i_t), GRUP.H_trials_subjMean{aa,freq}(i_t), GRUP.P_trials_subjMean{aa,freq}(i_t));
                end
