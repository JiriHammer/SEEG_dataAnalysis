function getTriggers_emoReco_stimuliTypes(params)
...
for sess = 1:size(params.storage.sessionCacheFiles,2)
    ...
    for tr = 1:nTr
        for stim = 1:size(trials(tr).stimulus,2)
            if strcmp(params.thisCategory, trials(tr).stimulus(stim).class)
                trial_event(tr) = 1;      % it is enough, if it appears at least once in stimuli (in case of concordance paradigm)
            end
        end       
    end    
...
%         if strcmp(trials(tr).stimulus(1).same_diff, 'same')
%         
%             % trigger times
...
%                     
%             % class labels
%             switch trials(tr).stimulus(1).class
%                case 'Fear'
%                    clz_label = cat(2, clz_label, 1);
%                case 'Happiness'
%                    clz_label = cat(2, clz_label, 2);
%                case 'Anger'
%                    clz_label = cat(2, clz_label, 3);
%                case 'Sadness'
%                    clz_label = cat(2, clz_label, 4);
%                case 'Disgust'
%                    clz_label = cat(2, clz_label, 5);                   
%                case 'Violin'
%                    clz_label = cat(2, clz_label, 6); 
%                case 'Piano'
%                    clz_label = cat(2, clz_label, 7); 
%                 otherwise
%                    clz_label = cat(2, clz_label, 666);
%             end                     
%         end
%     end
