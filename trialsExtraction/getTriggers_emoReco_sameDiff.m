function getTriggers_emoReco_sameDiff(params)
% finds events of concordant or discordant stimuli
...
for sess = 1:size(params.storage.sessionCacheFiles,2)
    ...
    % find events
    for tr = 1:nTr
        if isfield(trials(tr).stimulus(1), 'same_diff')
            if strcmp(params.thisCategory, trials(tr).stimulus(1).same_diff)
                trial_event(tr) = 1;
            end
        end
    end
...
%         elseif strcmp(trials(tr).stimulus(1).same_diff, 'diff')
