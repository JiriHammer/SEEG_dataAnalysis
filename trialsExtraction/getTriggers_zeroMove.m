function triggers = getTriggers_zeroMove(params)
...
for sess = 1:nSess
    ...
    normAccel = filterData(params, getStdSpeed(params, thisSess, 'absAcc'), filterSettings);
    %normSpeed = getStdSpeed(params, thisSess, 'absVel');    
    %normAccel = getStdSpeed(params, thisSess, 'absAcc');
    sessTime = [1:size(normSpeed,1)]'./params.amp.srate;
    sessSizes(sess) = size(normSpeed,1);
    pred{1,sess} = normSpeed; 
    pred{2,sess} = normAccel;
    i_zeroSpd = find(normSpeed < params.zeroSpeed.threshold);           % all in [std]
    i_zeroAcc = find(normAccel < params.zeroSpeed.threshold);           % all in [std]
    i_zero = intersect(i_zeroSpd, i_zeroAcc);                           % both zero speed & zero |acc| => zero movement 
...
if params.triggering.plotDetection
    ...
    for sess = 1:nSess                                                      % !!! sess-by-sess time series !!!
        ...
        hold on;
        plot(t, pred{1,sess}, 'r');                                                                   % in [std]
        plot(t, pred{2,sess}, 'b');                                                                   % in [std]
        ...
        for clz = binValues'
            ...
            for n = 1:length(i_clz)
                ...
                plot([trigEnd, trigEnd]./srate, get(gca,'ylim'), 'Color',clr, 'LineStyle','--');          % plot triggers end
            end   
        end
