function spectralTrials(params, resp, triggers)
...
rawTime = [0:size(resp{1},1)-1]./params.amp.srate;
...
assert(length(binCenters) == length(labels));
...
if ismember('classMeans', params.triggering.figType)
    for clz = 1:length(selLabels)
        ...
        for ch = 1:size(resp{1},2)
            ...
            for sess = 1:size(resp,2)
                rawData = resp{sess};
                rawTime = [0:size(rawData,1)-1]./params.amp.srate;
                timeAxis = downsample(rawTime, timeStep);   % in [s]
                ...
                for tr = 1:length(i_sess)
                    ...
                    if triggerPoint+params.triggering.time2cut(1) < 2.0 || triggerPoint+params.triggering.time2cut(2) > timeAxis(end) - 2.0
                        ...
                        if length(i_trial) - size(spectraTrials,2) == -1
                            i_trial = cat(2, i_trial, i_trial(end)+1);          % hack!
                        elseif length(i_trial) - size(spectraTrials,2) == 1
                            i_trial(end) = [];
                        end
                        assert(length(i_trial) == size(spectraTrials,2));
    ...
    % save spectra class means
    if ismember('classMeans', params.triggering.saveVals)
        save(params.storage.cacheFile, 'psdData', '-append');
    end
end

%% plot psd on freq for each class
if ismember('freqPower', params.triggering.figType)
...
%% TO DO: cut&save all trials (based on pauses!) for each chnl
