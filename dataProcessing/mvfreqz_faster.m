function [S,h,PDC,COH,DTF,DC,pCOH,dDTF,ffDTF, pCOH2, PDCF, coh,GGC,Af,GPDC]=mvfreqz_faster(B,A,C,N,Fs)
...
%
% ============================
% The algorithm was optimized: some for-cycles were replaced by the matrix operation   
...
%         end;   
        ...
        atmp=sum(reshape(A,[K1 K1 p+1]).*repmat(permute(exp(z*(0:p)*f(n)),[1 3 2]),[K1 K1 1]),3); % faster RJ
        % -----------------------------------------------------------------
...
idx=logical(repmat(eye(size(S,1)),[1 1 N]));
Sxx=repmat(reshape(S(idx),[1 K1 N]),[K1 1 1]);
SxxSyy=sqrt(abs(Sxx.*permute(Sxx,[2 1 3])));

S1xx=repmat(reshape(S1(idx),[1 K1 N]),[K1 1 1]);
S1xxS1yy=sqrt(abs(S1xx.*permute(S1xx,[2 1 3])));

Gxx=repmat(reshape(G(idx),[1 K1 N]),[K1 1 1]);
GxxGyy=Gxx.*permute(Gxx,[2 1 3]);

for k1=1:K1
    DEN=sum(abs(h(k1,:,:)).^2,2);
    
    COH(k1,:,:)=S(k1,:,:)./SxxSyy(k1,:,:);
    coh(k1,:,:)=S1(k1,:,:)./S1xxS1yy(k1,:,:);
    DTF(k1,:,:)= abs(h(k1,:,:))./sqrt(repmat(DEN,[1 K1 1]));
    ffDTF(k1,:,:) = abs(h(k1,:,:))./sqrt(repmat(sum(DEN,3),[1 K1 N]));
    pCOH2(k1,:,:) = abs(G(k1,:,:).^2)./GxxGyy(k1,:,:);
end
% -----------------------------------------------------------------------
...
%         DC = DC + A(:,k*K1+(1:K1)).^2;
% end;        
DC=sum(reshape(A(:,K1+1:end),[K1 K1 p]).^2,3); 
% -----------------------------------------------------------------------
...
Cxx=repmat(C(idx(:,:,1)),[1 K1]);
CxxCyy=Cxx.*permute(Cxx,[2 1]);

for k1=1:K1;
     GGC(k1,:,:)=((repmat(CxxCyy(k1,:),[1 1 N])-repmat(C(k1,:).^2,[1 1 N]))./repmat(permute(Cxx(:,k1,:),[2 1]),[1 1 N]))...
         .*real(h(k1,:,:).*conj(h(k1,:,:)))./abs(permute(Sxx(k1,:,:),[1 2 3]));
end
% -----------------------------------------------------------------------

pCOH=pCOH2; % missed in original code, edit by Janca
